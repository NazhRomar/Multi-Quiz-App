...
- `"fromCanvas": true` (top level) puts a graduation-cap icon next to the quiz on the home menu, with a tooltip saying the material came off the course’s Canvas page. It is about where the *source material* came from, not who wrote the questions, so it usually pairs with `"unofficial"` on a quiz built from posted slides. Leave it off for anything sourced elsewhere (NetAcad, a handout, an exam-answer site).
- `"unethicallySourced": true` (top level) puts a mask icon next to the quiz on the home menu — the codename for a quiz whose questions were lifted from a sitting of the real thing (a capture off a quiz site, a screenshot of someone’s attempt) rather than from material the course handed out. It is a provenance flag like `fromCanvas`, and in practice it rules `unofficial` out: the questions really are the course’s, they just were not given to you. Its tooltip and legend entry are deliberately uninformative (`???` / “Best not to ask.”) — the flag name is the codename, and the badge is not meant to explain itself on screen. Keep it that way.
